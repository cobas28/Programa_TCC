...
for i in range(popsize):
    ...
    pts_B = (0.5, np.random.uniform(0.5, min(1,M_center-0.1)),  1)
    B_center = pts_B[0]
    B_right = pts_B[1]
    B_left = pts_B[2]
...
#____________________________________Atualização das Velocidades e Posições_____________________________________________________________________________________________________________
    ...
    for i in range (popsize):
      for j in range (nder):
        ...
        if (j == 4):
          pts_derro[i,j] = max(pts_derro[i,1] + 0.01, pts_derro[i, 3], min(pts_derro[i,j]+vel_derro[i,j], pts_derro[i,5]))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]
        ...
        if (j == 7):
          pts_derro[i,j] = max(pts_derro[i,4] + 0.01, pts_derro[i, 6], min(pts_derro[i,j]+vel_derro[i,j],pts_derro[i,8]))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]
        ...
        if (j == 10):
          pts_derro[i,j] = max(pts_derro[i,7] + 0.01, pts_derro[i, 9], min(pts_derro[i,j]+vel_derro[i,j],pts_derro[i,11]))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]
    ...
    for i in range (popsize):
      for j in range (nsai):
        ...
        if (j == 5):
          pts_saida[i,j] = max(pts_saida[i,2] + 0.1, pts_saida[i, 4], min(pts_saida[i,j]+vel_saida[i,j], pts_saida[i,6]))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]
        ...
        if (j == 8):
          pts_saida[i,j] = max(pts_saida[i,5] + 0.1, pts_saida[i, 7], min(pts_saida[i,j]+vel_saida[i,j],pts_saida[i,9]))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]
        ...
        if (j == 11):
          pts_saida[i,j] = max(pts_saida[i,8] + 0.1, pts_saida[i, 10], min(pts_saida[i,j]+vel_saida[i,j],pts_saida[i,12]))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]
...
'''
with open('./dados6.pkl', 'wb') as arquivo:
    pickle.dump(y2, arquivo)
    
with open('./custo6.pkl', 'wb') as arquivo:
