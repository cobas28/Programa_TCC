...
#==========================Função Para definição dos minimos Globais==========================================================================================================

def globalMinimo(c, maxit):
    global_c = np.zeros(maxit) 
    global_c[0] = c[0]
    custo_min = c[0]
...
def calculo_tempos(y,tempo):
    ...
    y_final = 1  # Último valor da curva de controle
    tempo_subida = 0
...
with open('./dados1.pkl', 'rb') as arquivo:
    y1 = pickle.load(arquivo)

with open('./dados2.pkl', 'rb') as arquivo:
    y2 = pickle.load(arquivo)

with open('./dados3.pkl', 'rb') as arquivo:
    y3 = pickle.load(arquivo)

with open('./dados4.pkl', 'rb') as arquivo:
    y4 = pickle.load(arquivo)

with open('./dados5.pkl', 'rb') as arquivo:
    y5 = pickle.load(arquivo)

with open('./dados6.pkl', 'rb') as arquivo:
    y6 = pickle.load(arquivo)

# Carregar dados do arquivo de custos
with open('./custo1.pkl', 'rb') as arquivo:
    c1 = pickle.load(arquivo)

with open('./custo2.pkl', 'rb') as arquivo:
    c2 = pickle.load(arquivo)

with open('./custo3.pkl', 'rb') as arquivo:
    c3 = pickle.load(arquivo)

with open('./custo4.pkl', 'rb') as arquivo:
    c4 = pickle.load(arquivo)

with open('./custo5.pkl', 'rb') as arquivo:
    c5 = pickle.load(arquivo)

with open('./custo6.pkl', 'rb') as arquivo:
    c6 = pickle.load(arquivo)

global_c1 = np.zeros(maxit) 
global_c2 = np.zeros(maxit)
global_c3 = np.zeros(maxit)
global_c4 = np.zeros(maxit)
global_c5 = np.zeros(maxit)
global_c6 = np.zeros(maxit)

global_c1 = globalMinimo(c1, maxit)
global_c2 = globalMinimo(c2, maxit)
global_c3 = globalMinimo(c3, maxit)
global_c4 = globalMinimo(c4, maxit)
global_c5 = globalMinimo(c5, maxit)
global_c6 = globalMinimo(c6, maxit)
...
#Custo Minimo global
plt.figure()
plt.plot(global_c1,color='b', label='Cont. Fuzzy 1')
plt.plot(global_c2,color='g', label='Cont. Fuzzy 2')
plt.plot(global_c3,color='r', label='Cont. Fuzzy 3')
plt.plot(global_c4,color='c', label='Cont. Fuzzy 4')
plt.plot(global_c5,color='m', label='Cont. Fuzzy 5')
plt.plot(global_c6,color='y', label='Cont. Fuzzy 6')
plt.xlim(0, maxit)
plt.xlabel('Iterações')
plt.ylabel('Custo (IAE)')
plt.legend(loc='best', frameon=False)
