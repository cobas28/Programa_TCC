...
pop_global = np.zeros((popsize,ner))                    #Matrix que armazena as melhores posições encontradas para o erro
...
#____________________________________Atualização das Velocidades e Posições_____________________________________________________________________________________________________________
    ...
    for i in range(0,popsize):
      for j in range (0,ner):
        vel_erro[i,j] = (w*vel_erro[i,j] + c1*r1[i,j]*(local_erro[i,j] - pts_erro[i,j]) +c2*r2[i,j]*(pop_global[i,j] - pts_erro[i,j]))
...
pop_erro, pop_derro, pop_saida = func_pertinencia(pop_global, pts_derro, pts_saida)
