...
vel_derro = np.zeros((popsize, nder))                   #Vetor de Velocidades para a população do Derro
pts_saida = np.zeros((popsize, nsai))                   #Vetor de Posições para a população Saida
vel_saida = np.zeros((popsize, nsai))                   #Vetor de Velocidades para a população Saida

pop_global_erro = np.zeros((popsize,ner))               #Matrix que armazena as melhores posições encontradas para o erro
pop_global_derro = np.zeros((popsize,nder))             #Matrix que armazena as melhores posições encontradas para o derro
pop_global_saida = np.zeros((popsize,nsai))             #Matrix que armazena as melhores posições encontradas para a saida


for i in range(popsize):
    erro = control.Antecedent(np.arange(-1.0, 1.0, 0.001), 'erro')

    pts_Z = sorted([np.random.uniform(-0.375, 0.125), np.random.uniform(-0.25, 0.25), np.random.uniform(-0.125, 0.375)])
    Z_left = pts_Z[0]
    Z_center = pts_Z[1]
    Z_right = pts_Z[2]
    ...
    N_center = np.random.uniform(MN_centerR+0.1 , Z_center-0.1)
    ...
    P_center = np.random.uniform(Z_center+0.1, MP_centerL-0.1)
...
for i in range(popsize):
    ...
    N_center = np.random.uniform(MN_center+0.01 , Z_center-0.01)
    ...
    P_center = np.random.uniform(Z_center+0.01 , MP_center-0.01)
...
local_erro = np.copy(pts_erro)                  #Matriz que guardar as melhores posições adquiridas por cada membro da população erro
local_derro = np.copy(pts_derro)                #Matriz que guardar as melhores posições adquiridas por cada membro da população derro
local_saida = np.copy(pts_saida)                #Matriz que guardar as melhores posições adquiridas por cada membro da população saida
local_custo = np.copy(custo)                    #Matriz com os melhores custos adquiridos ate o momento por cada membro da população
...
for i in range(popsize):
    ...
    pop_global_derro[i] = pts_derro[indx]
    pop_global_saida[i] = pts_saida[indx]
...
for iter in range(1,maxit):
    ...
    r3 = np.random.rand(popsize,nder)
    r4 = np.random.rand(popsize,nder)
    r5 = np.random.rand(popsize,nsai)
    r6 = np.random.rand(popsize,nsai)
    a = np.copy(pts_erro)
    b = np.copy(pts_derro)
    c = np.copy(pts_saida)
...
#____________________________________Atualização das Velocidades e Posições_____________________________________________________________________________________________________________
    ...
    for i in range (popsize):
      for j in range (ner):
        ...
        if (j == 2):
          pts_erro[i,j] = max(pts_erro[i,1], min(pts_erro[i,j]+vel_erro[i,j], -0.15))
          vel_erro[i,j] = pts_erro[i,j] - a[i,j]

        if (j == 3):
          pts_erro[i,j] = max(-0.9, pts_erro[i,2], min(pts_erro[i,j]+vel_erro[i,j], 0))
          vel_erro[i,j] = pts_erro[i,j] - a[i,j]

        if (j == 4):
          pts_erro[i,j] = max(pts_erro[i,0], min(pts_erro[i,j]+vel_erro[i,j], pts_erro[i,3]))
          vel_erro[i,j] = pts_erro[i,j] - a[i,j]

        if (j == 5):
          pts_erro[i,j] = max(pts_erro[i,2] + 0.1, pts_erro[i, 4], min(pts_erro[i,j]+vel_erro[i,j], 0.2))
          vel_erro[i,j] = pts_erro[i,j] - a[i,j]

        if (j == 6):
          pts_erro[i,j] = max(pts_erro[i,3], pts_erro[i,5], min(pts_erro[i,j]+vel_erro[i,j],0.35))
          vel_erro[i,j] = pts_erro[i,j] - a[i,j]

        if (j == 7):
          pts_erro[i,j] = max(pts_erro[i,4], min(pts_erro[i,j]+vel_erro[i,j], pts_erro[i,6]))
          vel_erro[i,j] = pts_erro[i,j] - a[i,j]

        if (j == 8):
          pts_erro[i,j] = max(pts_erro[i,5] + 0.1, pts_erro[i, 7], min(pts_erro[i,j]+vel_erro[i,j],0.6))
          vel_erro[i,j] = pts_erro[i,j] - a[i,j]

        if (j == 9):
          pts_erro[i,j] = max(pts_erro[i,6], pts_erro[i,8], min(pts_erro[i,j]+vel_erro[i,j],0.75))
          vel_erro[i,j] = pts_erro[i,j] - a[i,j]

        if (j == 10):
          pts_erro[i,j] = max(pts_erro[i,7], min(pts_erro[i,j]+vel_erro[i,j], pts_erro[i,9]))
          vel_erro[i,j] = pts_erro[i,j] - a[i,j]

        if (j == 11):
          pts_erro[i,j] = max(pts_erro[i,8] + 0.1, pts_erro[i, 10], min(pts_erro[i,j]+vel_erro[i,j],0.8))
          vel_erro[i,j] = pts_erro[i,j] - a[i,j]

        if (j == 12):
          pts_erro[i,j] = max(pts_erro[i,9], pts_erro[i,11], min(pts_erro[i,j]+vel_erro[i,j], 1.0))
          vel_erro[i,j] = pts_erro[i,j] - a[i,j]

        if (j == 13):
          pts_erro[i,j] = max(pts_erro[i,10], min(pts_erro[i,j]+vel_erro[i,j], pts_erro[i,12]-0.1))
          vel_erro[i,j] = pts_erro[i,j] - a[i,j]

        if (j == 14):
          pts_erro[i,j] = max(pts_erro[i,11] + 0.1, pts_erro[i, 13], min(pts_erro[i,j]+vel_erro[i,j], 0.9))
          vel_erro[i,j] = pts_erro[i,j] - a[i,j]
    ...
    #Atualização do Derro
    for i in range (popsize):
      for j in range (nder):
        vel_derro[i,j] = (w*vel_derro[i,j] + c1*r3[i,j]*(local_derro[i,j] - pts_derro[i,j]) +c2*r4[i,j]*(pop_global_derro[i,j] - pts_derro[i,j]))

        if (j == 0):
          pts_derro[i,j] = -0.1
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]

        if (j == 1):
          pts_derro[i,j] = max(pts_derro[i,0], min(pts_derro[i,j]+vel_derro[i,j], -0.01))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]

        if (j == 2):
          pts_derro[i,j] = max(pts_derro[i,1]+0.01, min(pts_derro[i,j]+vel_derro[i,j], 0))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]

        if (j == 3):
          pts_derro[i,j] = max(pts_derro[i,0], min(pts_derro[i,j]+vel_derro[i,j], pts_derro[i,2]))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]

        if (j == 4):
          pts_derro[i,j] = max(pts_derro[i,1] + 0.01, pts_derro[i, 3], min(pts_derro[i,j]+vel_derro[i,j], 0.015))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]

        if (j == 5):
          pts_derro[i,j] = max(pts_derro[i,2], pts_derro[i,4], min(pts_derro[i,j]+vel_derro[i,j],0.025))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]

        if (j == 6):
          pts_derro[i,j] = max(pts_derro[i,3], min(pts_derro[i,j]+vel_derro[i,j], pts_derro[i,5]))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]

        if (j == 7):
          pts_derro[i,j] = max(pts_derro[i,4] + 0.01, pts_derro[i, 6], min(pts_derro[i,j]+vel_derro[i,j],0.04))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]

        if (j == 8):
          pts_derro[i,j] = max(pts_derro[i,5], pts_derro[i,7], min(pts_derro[i,j]+vel_derro[i,j],0.05))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]

        if (j == 9):
          pts_derro[i,j] = max(pts_derro[i,6], min(pts_derro[i,j]+vel_derro[i,j], pts_derro[i,8]))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]

        if (j == 10):
          pts_derro[i,j] = max(pts_derro[i,7] + 0.01, pts_derro[i, 9], min(pts_derro[i,j]+vel_derro[i,j],0.09))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]

        if (j == 11):
          pts_derro[i,j] = max(pts_derro[i,8], pts_derro[i,10], min(pts_derro[i,j]+vel_derro[i,j], 0.1))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]

        if (j == 12):
          pts_derro[i,j] = max(pts_derro[i,9], min(pts_derro[i,j]+vel_derro[i,j], pts_derro[i,11]-0.01))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]

        if (j == 13):
          pts_derro[i,j] = max(pts_derro[i,10] + 0.01, pts_derro[i, 12], min(pts_derro[i,j]+vel_derro[i,j], 0.1))
          vel_derro[i,j] = pts_derro[i,j] - b[i,j]
    ...
    #Atualização da Saida
    for i in range (popsize):
      for j in range (nsai):
        vel_saida[i,j] = (w*vel_saida[i,j] + c1*r5[i,j]*(local_saida[i,j] - pts_saida[i,j]) +c2*r6[i,j]*(pop_global_saida[i,j] - pts_saida[i,j]))

        if (j == 0):
          pts_saida[i,j] = 0.0
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]
        ...
        if (j == 2):
          pts_saida[i,j] = max(pts_saida[i,1], min(pts_saida[i,j]+vel_saida[i,j], 0.8))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]

        if (j == 3):
          pts_saida[i,j] = max(0.1,pts_saida[i,2], min(pts_saida[i,j]+vel_saida[i,j], 1))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]

        if (j == 4):
          pts_saida[i,j] = max(pts_saida[i,0], min(pts_saida[i,j]+vel_saida[i,j], pts_saida[i,3]))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]

        if (j == 5):
          pts_saida[i,j] = max(pts_saida[i,2] + 0.1, pts_saida[i, 4], min(pts_saida[i,j]+vel_saida[i,j], 1.15))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]

        if (j == 6):
          pts_saida[i,j] = max(pts_saida[i,3], pts_saida[i,5], min(pts_saida[i,j]+vel_saida[i,j],1.25))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]

        if (j == 7):
          pts_saida[i,j] = max(pts_saida[i,4], min(pts_saida[i,j]+vel_saida[i,j], pts_saida[i,6]))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]

        if (j == 8):
          pts_saida[i,j] = max(pts_saida[i,5] + 0.1, pts_saida[i, 7], min(pts_saida[i,j]+vel_saida[i,j],1.4))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]

        if (j == 9):
          pts_saida[i,j] = max(pts_saida[i,6], pts_saida[i,8], min(pts_saida[i,j]+vel_saida[i,j],1.5))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]

        if (j == 10):
          pts_saida[i,j] = max(pts_saida[i,7], min(pts_saida[i,j]+vel_saida[i,j], pts_saida[i,9]))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]

        if (j == 11):
          pts_saida[i,j] = max(pts_saida[i,8] + 0.1, pts_saida[i, 10], min(pts_saida[i,j]+vel_saida[i,j],1.8))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]

        if (j == 12):
          pts_saida[i,j] = max(pts_saida[i,9], pts_saida[i,11], min(pts_saida[i,j]+vel_saida[i,j], 2.0))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]

        if (j == 13):
          pts_saida[i,j] = max(pts_saida[i,10], min(pts_saida[i,j]+vel_saida[i,j], pts_saida[i,12]-0.1))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]

        if (j == 14):
          pts_saida[i,j] = max(pts_saida[i,11] + 0.1, pts_saida[i, 13], min(pts_saida[i,j]+vel_saida[i,j], 1.9))
          vel_saida[i,j] = pts_saida[i,j] - c[i,j]
...
#__________________________________________Atualizando as variavés locais_________________________________________________________________________________________________________
    ...
    for k in range (popsize):
        if (melhor_custo[k] == 1):
            ...
            local_derro[k]=pts_derro[k]
            local_saida[k]=pts_saida[k]
...
#_______________________________________Atualizando as variaveis globais___________________________________________________________________________________________________________
    ...
    if (custo_min_local < global_custo):
        for i in range(popsize):
          ...
          pop_global_derro[i] = pts_derro[ind]
          pop_global_saida[i] = pts_saida[ind]
        indx = ind
...
pop_erro, pop_derro, pop_saida = func_pertinencia(pop_global_erro, pop_global_derro, pop_global_saida)
...
'''
with open('./dados3.pkl', 'wb') as arquivo:
    pickle.dump(y2, arquivo)
    
with open('./custo3.pkl', 'wb') as arquivo:
