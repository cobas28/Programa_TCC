...
def discretizacao(A, B, C, D, ordem, T, n):
    ...
    for k in range (1,n,1):
        x1[k]=Sig[0,0]*x1[k-1]+ Sig[0,1]*x2[k-1] + Sig[0,2]*x3[k-1] + B[0,0]*T*u[k-1] 
        x2[k]=Sig[1,0]*x1[k-1]+ Sig[1,1]*x2[k-1] + Sig[1,2]*x3[k-1] + B[1,0]*T*u[k-1] 
        x3[k]=Sig[2,0]*x1[k-1]+ Sig[2,1]*x2[k-1] + Sig[2,2]*x3[k-1] + B[2,0]*T*u[k-1] 
        y[k]=C[0,0]*x1[k] + C[0,1]*x2[k] + C[0,2]*x3[k]
        tempo[k]=tempo[k-1]+T
    return y
...
def calculaCusto(A, B, C, D, ordem, T, n, erro, derro, saida):
    ...
    #Definição das variáveis de entrada e saída do controlador Fuzzy
...
def simula_controle(erro, derro, saida, y1):
    ...
    #Definição das variáveis e saída do controlador Fuzzy
...
#Planta de controle (n=3)
ordem = 3
A = np.array([(0,1,0),(0,0,1),(-24,-24,-8)])
B = np.array([(0,),(0,),(1,)])
C = np.array([(24, -12, 2)])
D = 0
...
#_____________________________________Aplicando mutação na população________________________________________________________________________________________________________________________________________________________________________
    ...
    mut_col_erro = np.random.randint(2,Nter-2,(nmut,3))                       
    ...
    mut_col_derro = np.random.randint(1,Ntder-1,(nmut,3))             
    ...
    mut_lin_saida = np.sort(np.random.randint(6,popsize,nmut))           
    mut_col_saida = np.random.randint(2,Ntsai-2,(nmut,3))                
...
with open('./novocusto3.pkl', 'wb') as arquivo:
