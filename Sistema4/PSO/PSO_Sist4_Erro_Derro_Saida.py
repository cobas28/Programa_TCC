...
def calculaCusto(A, B, C, D, ordem, T, n, erro, derro, saida):
    ...
    regra14= control.Rule(((~erro['MN']) & (~erro['N']) & (~erro['Z']) & (~erro['P']) & (~erro['MP']))
                          |((~derro['MN']) & (~derro['N']) & (~derro['Z']) & (~derro['P']) & (~derro['MP'])), saida['MB'])
...
for i in range(popsize):
    ...
    pts_M = sorted([np.random.uniform(0.5, 1.3), np.random.uniform(0.5, 1.5), np.random.uniform(0.7, 1.5)])
    M_left = pts_M[0]
    M_center = pts_M[1]
    M_right = pts_M[2]
...
'''
with open('./novodados9.pkl', 'wb') as arquivo:
    pickle.dump(y2, arquivo)
    
with open('./novocusto9.pkl', 'wb') as arquivo:
