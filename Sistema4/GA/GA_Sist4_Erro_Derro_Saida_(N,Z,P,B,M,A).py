...
custo = np.zeros([popsize, 1])                  #Matriz que recebe os valores de custo referente ao IAE
y = np.zeros((maxit,n))
...
for i in range(popsize):
    ...
    pts_N = (-0.1, np.random.uniform(-0.1, min(0,Z_center-0.01)),  0)
    N_left = pts_N[0]
    M_center = pts_N[1]
    N_right = pts_N[2]
    
    pts_P = (0, np.random.uniform(max(0,Z_center+0.01), 0.1),  0.1)
    P_left = pts_P[0]
    P_center = pts_P[1]
    P_right = pts_P[2]
...
for i in range(popsize):
    ...
    pts_M = (0.75, np.random.uniform(0.75, 1.25),  1.25)
    M_left = pts_M[0]
    M_center = pts_M[1]
    M_right = pts_M[2]
    
    pts_B = (0.5, np.random.uniform(0.6, min(1,M_center-0.1)),  1)
    B_center = pts_B[0]
    B_right = pts_B[1]
    B_left = pts_B[2]
    ...
    pts_A = (1, np.random.uniform(max(1,M_center+0.1), 1.4),  1.5)
    A_left = pts_A[0]
    A_center = pts_A[1]
    A_right = pts_A[2]
...
#_____________________________________Aplicando mutação na população________________________________________________________________________________________________________________________________________________________________________
    ...
    mut_col_saida = np.random.choice(escolhas, nmut)                     

    
    for i in range(0,nmut):
        
        
        if(mut_col_saida[i] == 5):
            pts_saida[mut_lin_saida[i],mut_col_saida[i]]= np.random.uniform(max(pts_saida[mut_lin_saida[i],2],
                                                                                pts_saida[mut_lin_saida[i],4]),
                                                                            min(pts_saida[mut_lin_saida[i],8],
                                                                                pts_saida[mut_lin_saida[i],6]))            
                   
        if(mut_col_saida[i] == 8):
            pts_saida[mut_lin_saida[i],mut_col_saida[i]]= np.random.uniform(max(pts_saida[mut_lin_saida[i],5],
                                                                                pts_saida[mut_lin_saida[i],7]),
                                                                            min(pts_saida[mut_lin_saida[i],9],
                                                                                pts_saida[mut_lin_saida[i],11]))
        
        if(mut_col_saida[i] == 11):
            pts_saida[mut_lin_saida[i],mut_col_saida[i]]= np.random.uniform(max(pts_saida[mut_lin_saida[i],8],
...
'''
with open('./novodados12.pkl', 'wb') as arquivo:
    pickle.dump(y2, arquivo)

with open('./novocusto12.pkl', 'wb') as arquivo:
