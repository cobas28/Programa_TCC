...
def calculaCusto(A, B, C, D, ordem, T, n, erro, derro, saida):
    ...
    regra4  = control.Rule(erro['N']  & (derro['Z'] | derro['N'])                , saida['MB'])
    regra5  = control.Rule(erro['N']  &  derro['MN']                             , saida['B'])
    regra6  = control.Rule(erro['Z']  & (derro['MN'] | derro['N'])               , saida['B'])
    regra7  = control.Rule(erro['Z']  & derro['Z']                               , saida['M'])
    regra8  = control.Rule(erro['Z']  & (derro['MP'] | derro['P'])               , saida['A'])
...
#Simulação em malha aberta da planta
...
selection= 0.4                                        # Fração da população que será mantida
...
for i in range(popsize):
    ...
    pts_Z = sorted([np.random.uniform(-0.5, 0.4), np.random.uniform(-0.5, 0.5), np.random.uniform(-0.4, 0.5)])
    Z_left = pts_Z[0]
    Z_center = pts_Z[1]
    Z_right = pts_Z[2]
...
for i in range(popsize):
    ...
    pts_Z = sorted([np.random.uniform(-0.05, 0.04), np.random.uniform(-0.05, 0.05), np.random.uniform(-0.04, 0.05)])
    Z_left = pts_Z[0]
    Z_center = pts_Z[1]
    Z_right = pts_Z[2]
    ...
    MN_center = np.random.uniform(-0.1, MN_right)
    pts_MN = (MN_left, MN_center, MN_right)
    
    MP_right = 0.1
    MP_left = np.random.uniform(Z_right,0.09)
    MP_center = np.random.uniform(MP_left, 0.1)
...
for i in range(popsize):
    ...
    pts_M = sorted([np.random.uniform(0.5, 1.4), np.random.uniform(0.5, 1.5), np.random.uniform(0.6, 1.5)])
    M_left = pts_M[0]
    M_center = pts_M[1]
    M_right = pts_M[2]
    ...
    MB_left = 0
    MB_centerL = 0
    MB_right = np.random.uniform(0.1,M_left)
    MB_centerR = np.random.uniform(MB_left, MB_right)
    pts_MB = sorted([MB_left, MB_centerL,MB_centerR, MB_right])
    
    MA_right = 2
    MA_centerR = 2
    MA_left = np.random.uniform(M_right,1.9)
    MA_centerL = np.random.uniform(MA_left, MA_centerR)
    pts_MA = sorted([MA_left, MA_centerL,MA_centerR, MA_right])
    
    B_center = np.random.uniform(MB_centerR , M_center)
    B_right = max(MB_right,max(B_center, np.random.uniform(M_left,M_right)))
    B_left = min(M_left,min(B_center, np.random.uniform(MB_centerL, MB_right)))
    pts_B = (B_left, B_center, B_right)
    
    
    A_center = np.random.uniform(M_center, MA_centerL)
    A_left = min(MA_left,min(A_center, np.random.uniform(M_left, M_right)))
    A_right = max(M_right,max(A_center, np.random.uniform(MA_left,MA_centerL)))
...
for i in indices:               #reorganiza as populações de acordo com os valores de custo
    pts_erro[j] = a[i]
    pts_derro[j] = b[i]
    pts_saida[j] = c[i]
    j+=1
...
custo_min = np.zeros(maxit)                 #array que contém os valores minimos de custo para cada iteração
custo_min[0] = np.min(custo)                
melhor_custo_min = custo_min[0]             #guarda o menor custo minimo encontrado 


melhor_min_pop_erro = np.zeros((popsize, ner))       #Guarda a população erro que gerou o menor custo    
melhor_min_pop_derro = np.zeros((popsize, nder))     #Guarda a população derro que gerou o menor custo    
melhor_min_pop_saida = np.zeros((popsize, nsai))     #Guarda a população saida que gerou o menor custo 
 

melhor_min_pop_erro = np.copy(pts_erro)              #Guarda a população erro que gerou o menor custo    
melhor_min_pop_derro = np.copy(pts_derro)            #Guarda a população derro que gerou o menor custo    
melhor_min_pop_saida = np.copy(pts_saida)            #Guarda a população saida que gerou o menor custo 
       

melhor_geracao_min = 0                          #Guarda a melhor geração
melhor_custo = 0                                #Guarda o meçhor custo
...
#_________________________________________Cruzamento aritimético____________________________________________________________________________________________________________________________________________________________________________
    ...
    for ix in range(popsize-1, keep-1,-2):
        pts_erro[ix,:] = alpha*copia_erro[ma[ic],:] + (1-alpha)*copia_erro[pa[ic],:]      #1º Filho
        
        if(ix-1>=keep):
            pts_erro[ix-1,:] = alpha*copia_erro[pa[ic],:] + (1-alpha)*copia_erro[ma[ic],:]   #2º Filho
        ic+=1
    ...
    for ix in range(popsize-1, keep-1,-2):
        pts_derro[ix,:] = alpha*copia_derro[ma[ic],:] + (1-alpha)*copia_derro[pa[ic],:]      #1º Filho
        
        if(ix-1>=keep):
            pts_derro[ix-1,:] = alpha*copia_derro[pa[ic],:] + (1-alpha)*copia_derro[ma[ic],:]   #2º Filho
        ic+=1
    
    #Cruzamento da população Saida    
    ic=0
    alpha = np.random.uniform(0, 1)          
    copia_saida = np.copy(pts_saida)           
    for ix in range(popsize-1, keep-1,-2):
        pts_saida[ix,:] = alpha*copia_saida[ma[ic],:] + (1-alpha)*copia_saida[pa[ic],:]      #1º Filho
        
        if(ix-1>=keep):
            pts_saida[ix-1,:] = alpha*copia_saida[pa[ic],:] + (1-alpha)*copia_saida[ma[ic],:]   #2º Filho
        ic+=1
...
#_____________________________________Aplicando mutação na população________________________________________________________________________________________________________________________________________________________________________
    ...
    mut_lin_erro = np.sort(np.random.randint(4,popsize,nmut))                 
    mut_col_erro = np.random.randint(2,Nter-2,(nmut,1))                       
   
   
    for i in range(0,nmut):
        for j in range(0,1):
            if(mut_col_erro[i,j] == 0):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= -1.0
            
            if(mut_col_erro[i,j] == 1):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= -1.0
           
            if(mut_col_erro[i,j] == 2):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= np.random.uniform(pts_erro[mut_lin_erro[i],1],
                                                                                min(pts_erro[mut_lin_erro[i],3],
                                                                                    pts_erro[mut_lin_erro[i],5]))
            if(mut_col_erro[i,j] == 3):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= np.random.uniform(max(pts_erro[mut_lin_erro[i],2],
                                                                                    pts_erro[mut_lin_erro[i],4]),
                                                                                pts_erro[mut_lin_erro[i],6])
            if(mut_col_erro[i,j] == 4):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= np.random.uniform(pts_erro[mut_lin_erro[i],0],
                                                                                min(pts_erro[mut_lin_erro[i],3],
                                                                                    pts_erro[mut_lin_erro[i],5],
                                                                                    pts_erro[mut_lin_erro[i],7]))
            if(mut_col_erro[i,j] == 5):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= np.random.uniform(max(pts_erro[mut_lin_erro[i],2],
                                                                                    pts_erro[mut_lin_erro[i],4]),
                                                                                min(pts_erro[mut_lin_erro[i],8],
                                                                                    pts_erro[mut_lin_erro[i],6]))            
            if(mut_col_erro[i,j] == 6):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]=  np.random.uniform(max(pts_erro[mut_lin_erro[i],5],
                                                                                    pts_erro[mut_lin_erro[i],7]),
                                                                                pts_erro[mut_lin_erro[i],9])                
            if(mut_col_erro[i,j] == 7):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= np.random.uniform(pts_erro[mut_lin_erro[i],4],
                                                                                min(pts_erro[mut_lin_erro[i],6],
                                                                                    pts_erro[mut_lin_erro[i],8],
                                                                                    pts_erro[mut_lin_erro[i],10]))           
            if(mut_col_erro[i,j] == 8):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= np.random.uniform(max(pts_erro[mut_lin_erro[i],5],
                                                                                    pts_erro[mut_lin_erro[i],7]),
                                                                                min(pts_erro[mut_lin_erro[i],9],
                                                                                    pts_erro[mut_lin_erro[i],11]))
            if(mut_col_erro[i,j] == 9):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= np.random.uniform(max(pts_erro[mut_lin_erro[i],8],
                                                                                    pts_erro[mut_lin_erro[i],10],
                                                                                    pts_erro[mut_lin_erro[i],6]),
                                                                                pts_erro[mut_lin_erro[i],12])            
            if(mut_col_erro[i,j] == 10):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= np.random.uniform(pts_erro[mut_lin_erro[i],7],
                                                                                min(pts_erro[mut_lin_erro[i],9],
                                                                                    pts_erro[mut_lin_erro[i],11],
                                                                                    pts_erro[mut_lin_erro[i],13]))
            if(mut_col_erro[i,j] == 11):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= np.random.uniform(max(pts_erro[mut_lin_erro[i],8],
                                                                                    pts_erro[mut_lin_erro[i],10]),
                                                                                min(pts_erro[mut_lin_erro[i],12],
                                                                                    pts_erro[mut_lin_erro[i],14]))                
            if(mut_col_erro[i,j] == 12):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= np.random.uniform(max(pts_erro[mut_lin_erro[i],11],
                                                                                    pts_erro[mut_lin_erro[i],13],
                                                                                    pts_erro[mut_lin_erro[i],9]),
                                                                                pts_erro[mut_lin_erro[i],16])                
            if(mut_col_erro[i,j] == 13):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= np.random.uniform(pts_erro[mut_lin_erro[i],10],
                                                                                min(pts_erro[mut_lin_erro[i],12],
                                                                                    pts_erro[mut_lin_erro[i],14]))                
            if(mut_col_erro[i,j] == 14):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= np.random.uniform(max(pts_erro[mut_lin_erro[i],11],
                                                                                    pts_erro[mut_lin_erro[i],13]),
                                                                                pts_erro[mut_lin_erro[i],15])                
            if(mut_col_erro[i,j] == 15):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= 1.0
                                                                             
            if(mut_col_erro[i,j] == 16):
                pts_erro[mut_lin_erro[i],mut_col_erro[i,j]]= 1.0
    
            
    #Mutação para população derro
    mut_lin_derro = np.sort(np.random.randint(4,popsize,nmut))        
    mut_col_derro = np.random.randint(1,Ntder-1,(nmut,1))             

    
    for i in range(0,nmut):
        for j in range(0,1):
            if(mut_col_derro[i,j] == 0):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= -0.1
                
            if(mut_col_derro[i,j] == 1):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= np.random.uniform(pts_derro[mut_lin_derro[i],0],
                                                                                  min(pts_derro[mut_lin_derro[i],2],
                                                                                      pts_derro[mut_lin_derro[i],4]))
            
            if(mut_col_derro[i,j] == 2):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= np.random.uniform(max(pts_derro[mut_lin_derro[i],1],
                                                                                      pts_derro[mut_lin_derro[i],3]),
                                                                                  pts_derro[mut_lin_derro[i],5])
            if(mut_col_derro[i,j] == 3):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= np.random.uniform(pts_derro[mut_lin_derro[i],0],
                                                                                  min(pts_derro[mut_lin_derro[i],2],
                                                                                      pts_derro[mut_lin_derro[i],4],
                                                                                      pts_derro[mut_lin_derro[i],6]))
            
            if(mut_col_derro[i,j] == 4):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= np.random.uniform(max(pts_derro[mut_lin_derro[i],1],
                                                                                      pts_derro[mut_lin_derro[i],3]),
                                                                                  min(pts_derro[mut_lin_derro[i],5],
                                                                                      pts_derro[mut_lin_derro[i],7]))
            
            if(mut_col_derro[i,j] == 5):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= np.random.uniform(max(pts_derro[mut_lin_derro[i],4],
                                                                                      pts_derro[mut_lin_derro[i],6],
                                                                                      pts_derro[mut_lin_derro[i],2]),
                                                                                  pts_derro[mut_lin_derro[i],8])
                
            if(mut_col_derro[i,j] == 6):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= np.random.uniform(pts_derro[mut_lin_derro[i],3],
                                                                                  min(pts_derro[mut_lin_derro[i],5],
                                                                                      pts_derro[mut_lin_derro[i],7],
                                                                                      pts_derro[mut_lin_derro[i],9]))
            
            if(mut_col_derro[i,j] == 7):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= np.random.uniform(max(pts_derro[mut_lin_derro[i],4],
                                                                                      pts_derro[mut_lin_derro[i],6]),
                                                                                  min(pts_derro[mut_lin_derro[i],8],
                                                                                      pts_derro[mut_lin_derro[i],10]))
            if(mut_col_derro[i,j] == 8):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= np.random.uniform(max(pts_derro[mut_lin_derro[i],7],
                                                                                      pts_derro[mut_lin_derro[i],9],
                                                                                      pts_derro[mut_lin_derro[i],5]),
                                                                                  pts_derro[mut_lin_derro[i],11])
            
            if(mut_col_derro[i,j] == 9):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= np.random.uniform(pts_derro[mut_lin_derro[i],6],
                                                                                  min(pts_derro[mut_lin_derro[i],8],
                                                                                      pts_derro[mut_lin_derro[i],10],
                                                                                      pts_derro[mut_lin_derro[i],12]))
                
            if(mut_col_derro[i,j] == 10):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= np.random.uniform(max(pts_derro[mut_lin_derro[i],7],
                                                                                      pts_derro[mut_lin_derro[i],9]),
                                                                                  min(pts_derro[mut_lin_derro[i],11],
                                                                                      pts_derro[mut_lin_derro[i],13]))
                
            if(mut_col_derro[i,j] == 11):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= np.random.uniform(max(pts_derro[mut_lin_derro[i],10],
                                                                                      pts_derro[mut_lin_derro[i],12],
                                                                                      pts_derro[mut_lin_derro[i],8]),
                                                                                  pts_derro[mut_lin_derro[i],14])
                
            if(mut_col_derro[i,j] == 12):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= np.random.uniform(pts_derro[mut_lin_derro[i],9],
                                                                                  min(pts_derro[mut_lin_derro[i],11],
                                                                                      pts_derro[mut_lin_derro[i],13]))
                
            if(mut_col_derro[i,j] == 13):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= np.random.uniform(max(pts_derro[mut_lin_derro[i],10],
                                                                                      pts_derro[mut_lin_derro[i],12]),
                                                                                  pts_derro[mut_lin_derro[i],14])
                
            if(mut_col_derro[i,j] == 14):
                pts_derro[mut_lin_derro[i],mut_col_derro[i,j]]= 0.1          
    
    # #Mutação para população saida
    mut_lin_saida = np.sort(np.random.randint(4,popsize,nmut))           
    mut_col_saida = np.random.randint(2,Ntsai-2,(nmut,1))                

    
    for i in range(0,nmut):
        for j in range(0,1):
            if(mut_col_saida[i,j] == 0):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= 0
            
            if(mut_col_saida[i,j] == 1):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= 0
            
            if(mut_col_saida[i,j] == 2):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= np.random.uniform(pts_saida[mut_lin_saida[i],1],
                                                                                min(pts_saida[mut_lin_saida[i],3],
                                                                                    pts_saida[mut_lin_saida[i],5]))
            if(mut_col_saida[i,j] == 3):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= np.random.uniform(max(pts_saida[mut_lin_saida[i],2],
                                                                                    pts_saida[mut_lin_saida[i],4]),
                                                                                pts_saida[mut_lin_saida[i],6])
            if(mut_col_saida[i,j] == 4):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= np.random.uniform(pts_saida[mut_lin_saida[i],0],
                                                                                min(pts_saida[mut_lin_saida[i],3],
                                                                                    pts_saida[mut_lin_saida[i],5],
                                                                                    pts_saida[mut_lin_saida[i],7]))
            if(mut_col_saida[i,j] == 5):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= np.random.uniform(max(pts_saida[mut_lin_saida[i],2],
                                                                                    pts_saida[mut_lin_saida[i],4]),
                                                                                min(pts_saida[mut_lin_saida[i],8],
                                                                                    pts_saida[mut_lin_saida[i],6]))            
            if(mut_col_saida[i,j] == 6):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]=  np.random.uniform(max(pts_saida[mut_lin_saida[i],5],
                                                                                    pts_saida[mut_lin_saida[i],7],
                                                                                    pts_saida[mut_lin_saida[i],3]),
                                                                                pts_saida[mut_lin_saida[i],9])                
            if(mut_col_saida[i,j] == 7):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= np.random.uniform(pts_saida[mut_lin_saida[i],4],
                                                                                min(pts_saida[mut_lin_saida[i],6],
                                                                                    pts_saida[mut_lin_saida[i],8],
                                                                                    pts_saida[mut_lin_saida[i],10]))           
            if(mut_col_saida[i,j] == 8):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= np.random.uniform(max(pts_saida[mut_lin_saida[i],5],
                                                                                    pts_saida[mut_lin_saida[i],7]),
                                                                                min(pts_saida[mut_lin_saida[i],9],
                                                                                    pts_saida[mut_lin_saida[i],11]))
            if(mut_col_saida[i,j] == 9):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= np.random.uniform(max(pts_saida[mut_lin_saida[i],8],
                                                                                    pts_saida[mut_lin_saida[i],10],
                                                                                    pts_saida[mut_lin_saida[i],6]),
                                                                                pts_saida[mut_lin_saida[i],12])            
            if(mut_col_saida[i,j] == 10):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= np.random.uniform(pts_saida[mut_lin_saida[i],7],
                                                                                min(pts_saida[mut_lin_saida[i],9],
                                                                                    pts_saida[mut_lin_saida[i],11],
                                                                                    pts_saida[mut_lin_saida[i],13]))
            if(mut_col_saida[i,j] == 11):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= np.random.uniform(max(pts_saida[mut_lin_saida[i],8],
                                                                                    pts_saida[mut_lin_saida[i],10]),
                                                                                min(pts_saida[mut_lin_saida[i],12],
                                                                                    pts_saida[mut_lin_saida[i],14]))                
            if(mut_col_saida[i,j] == 12):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= np.random.uniform(max(pts_saida[mut_lin_saida[i],11],
                                                                                    pts_saida[mut_lin_saida[i],13],
                                                                                    pts_saida[mut_lin_saida[i],9]),
                                                                                pts_saida[mut_lin_saida[i],16])                
            if(mut_col_saida[i,j] == 13):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= np.random.uniform(pts_saida[mut_lin_saida[i],10],
                                                                                min(pts_saida[mut_lin_saida[i],12],
                                                                                    pts_saida[mut_lin_saida[i],14]))                
            if(mut_col_saida[i,j] == 14):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= np.random.uniform(max(pts_saida[mut_lin_saida[i],11],
                                                                                    pts_saida[mut_lin_saida[i],13]),
                                                                                pts_saida[mut_lin_saida[i],15])                
            if(mut_col_saida[i,j] == 15):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= 2.0                                                                            
            
            if(mut_col_saida[i,j] == 16):
                pts_saida[mut_lin_saida[i],mut_col_saida[i,j]]= 2.0
...
#_____________________________Avaliação das População apos o cruzamento e mutações______________________________________________________________________________________________________________________________________________________
    ...
    print('Custos:', custo)
#_________________________________Analisando os minimos e media para iteração atual_____________________________________________________________________________________________________________________________________________________

    custo_min[iga] = np.min(custo)                

    if(custo_min[iga]<melhor_custo_min).any():
...
#___________________________________________Critério de parada__________________________________________________________________________________________________________________________________________________________________________    
    
    
    if (custo_min[iga]<0.01).any():
        break

#______________________________________________Resultados Finais________________________________________________________________________________________________________________________________________________________________________
...
#Salva os dados referente a melhor curva encontrada e a evolução do custo
'''
with open('./novodados3.pkl', 'wb') as arquivo:
    pickle.dump(y2, arquivo)
    
with open('./novocusto33.pkl', 'wb') as arquivo:
