...
maxit = 200                                           # Total de iterações
razao_resfriamento = 0.97                             # Taxa de Resfriamento
temperatura_inicial = 200                             # Temperatura Inicial
...
B_center = np.random.uniform(MB_centerR+0.01 , M_center-0.01)
...
A_center = np.random.uniform(M_center+0.01, MA_centerL-0.01)
...
melhor_derro = np.copy(pts_derro)           # Armazena a solução derro que gerou o menor custo
melhor_saida = np.copy(pts_saida)           # Armazena a solução saída que gerou o menor custo
...
#__________________________________________________Calculando uma nova solução vizinha__________________________________________________________________________________________________
    ...
    #Solução vizinha saida 
    ncol_saida = np.random.randint(2,15,nvar)                       #escolhe aleatoriamente colunas para serem modificadas
   
    for i in range(0,nvar):
        
        if(ncol_saida[i] == 0):
            novo_pts_saida[0]= 0.0
        
        if(ncol_saida[i] == 1):
            novo_pts_saida[1]= 0.0
       
        if(ncol_saida[i] == 2):
            novo_pts_saida[2]= np.random.uniform(novo_pts_saida[1],min(novo_pts_saida[3], novo_pts_saida[5]))
        
        if(ncol_saida[i] == 3):
            novo_pts_saida[3]= np.random.uniform(max(novo_pts_saida[2], novo_pts_saida[4]), novo_pts_saida[6])
        
        if(ncol_saida[i] == 4):
            novo_pts_saida[4]= np.random.uniform(novo_pts_saida[0], min(novo_pts_saida[3], novo_pts_saida[5], novo_pts_saida[7]))
        
        if(ncol_saida[i] == 5):
            novo_pts_saida[5]= np.random.uniform(max(novo_pts_saida[2], novo_pts_saida[4]), min(novo_pts_saida[8], novo_pts_saida[6]))            
        
        if(ncol_saida[i] == 6):
            novo_pts_saida[6]=  np.random.uniform(max(novo_pts_saida[5], novo_pts_saida[7]), novo_pts_saida[9])                
       
        if(ncol_saida[i] == 7):
            novo_pts_saida[7]= np.random.uniform(novo_pts_saida[4], min(novo_pts_saida[6], novo_pts_saida[8], novo_pts_saida[10]))           
        
        if(ncol_saida[i] == 8):
            novo_pts_saida[8]= np.random.uniform(max(novo_pts_saida[5], novo_pts_saida[7]), min(novo_pts_saida[9], novo_pts_saida[11]))
        
        if(ncol_saida[i] == 9):
            novo_pts_saida[9]= np.random.uniform(max(novo_pts_saida[8], novo_pts_saida[10], novo_pts_saida[6]), novo_pts_saida[12])            
        
        if(ncol_saida[i] == 10):
            novo_pts_saida[10]= np.random.uniform(novo_pts_saida[7], min(novo_pts_saida[9], novo_pts_saida[11], novo_pts_saida[13]))
        
        if(ncol_saida[i] == 11):
            novo_pts_saida[11]= np.random.uniform(max(novo_pts_saida[8], novo_pts_saida[10]), min(novo_pts_saida[12], novo_pts_saida[14]))                
        
        if(ncol_saida[i] == 12):
            novo_pts_saida[12]= np.random.uniform(max(novo_pts_saida[11], novo_pts_saida[13], novo_pts_saida[9]), novo_pts_saida[16])                
        
        if(ncol_saida[i] == 13):
            novo_pts_saida[13]= np.random.uniform(novo_pts_saida[10],min(novo_pts_saida[12], novo_pts_saida[14]))                
        
        if(ncol_saida[i] == 14):
            novo_pts_saida[14]= np.random.uniform(max(novo_pts_saida[11], novo_pts_saida[13]), novo_pts_saida[15])                
        
        if(ncol_saida[i] == 15):
            novo_pts_saida[15]= 2.0
                                                                         
        if(ncol_saida[i] == 16):
            novo_pts_saida[16]= 2.0
...
'''
with open('./novodados7.pkl', 'wb') as arquivo:
    pickle.dump(y2, arquivo)
    
with open('./novocusto7.pkl', 'wb') as arquivo:
